...
public class CountdownView extends TextView{
    ...
    private int mTime = 3000;   //默认计时
    private double mDrawTimes = 6;  //总的绘制次数
    private int mCurrentDrawTimes;  //已经绘制的次数
    private double mEachDrawAngle = 60; //默认每次绘制90度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float textY = mCenterY - (paint.descent() + paint.ascent()) / 2 - 5;
...
//        paint.setAntiAlias(true);  //防锯齿
//        paint.setTextAlign(Paint.Align.CENTER);
//        textY = mCenterY - (paint.descent() + paint.ascent()) / 2 + 24;
//        if (mCurrentDrawTimes % 2 ==0 ){
//            canvas.drawText((mDrawTimes - mCurrentDrawTimes)/2 + "", mCenterX, textY, paint);
//        }else{
//            canvas.drawText((mDrawTimes + 1 - mCurrentDrawTimes)/2 + "", mCenterX, textY, paint);
//        }
        ...
        canvas.drawArc(mArcRectF, -90,
                (float) ((mCurrentDrawTimes + 1) * mEachDrawAngle), false, mPaint);
    }
    ...
    public void star() {
        final double changePer = 100f / mDrawTimes;
