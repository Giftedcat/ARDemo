 ...
 */
package com.git.easylib.http.callback;
...
import com.git.easylib.EventMessage.TokenMessage;
import com.git.easylib.R;
import com.git.easylib.ui.dialog.WaitDialog;
...
public class HttpResponseListener<T> implements OnResponseListener<T> {
    ...
    public void onSucceed(int what, Response<T> response) {
        if (callback != null) {
            ...
            if(response.get() != null && ((BaseBean)response.get()).getResultCode() == 10010){
                //token 过期 失效
                EventBus.getDefault().post(new TokenMessage(((BaseBean)response.get()).getResultCode()));
            }
