package com.github.mikephil.charting.photopicker.utils;
...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(mContext, "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(mContext),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
